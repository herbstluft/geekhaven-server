...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if(!empty($validacion)){
    header("Location:/src/views/admin/html/editNomUniverso.php?mensaje=failed&nom=$universo&id=$id");
  }
  else{
   $insertQry = "UPDATE `universo`SET`universo`='$universo' WHERE universo.id_universo=$id";
    $inertar=$db->ejecutarConsulta($insertQry);
    header("Location:/src/views/admin/html/editNomUniverso.php?mensaje=success&id=$id&nom=$universo");
  }
} 
