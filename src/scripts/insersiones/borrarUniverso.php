...
if(!empty($ValidarProductos)){
  header("Location:/src/views/admin/html/editUniverso.php?mensaje=failed&uni=$universo");
}
else{
$deleteUniversoQry="DELETE FROM `universo` WHERE `id_universo`='$universo'";
$deleteUniverso=$db->ejecutarConsulta($deleteUniversoQry);

header("Location:/src/views/admin/html/editUniverso.php?mensaje=success&uni=$universo");
}
}
?>
  </body>
