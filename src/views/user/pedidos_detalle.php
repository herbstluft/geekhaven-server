...
?>
            ...
            <div class="row">
                ...
                $SacarImagenesQry="SELECT * from productos INNER JOIN img_productos on img_productos.id_producto=productos.id_producto where productos.id_producto=$id_producto";
                $SacarImagenes=$db1->seleccionarDatos($SacarImagenesQry);
                foreach($SacarImagenes as $img){
                    $imgpd = $img['nombre_imagen'];}
                    
                    if(!empty($imgpd)){ ?>
                    <img src="/src/views/admin/html/img_producto/<?php echo $imgpd ?>" class="d-block" width="110"  height="110px" alt="...">
                        <?php
                    }else{ ?>
...
<center><a href="http://<?php echo $HOST?>/src/scripts/tickets/loading_ticket.php?id_orden=<?php echo $id_orden?>&id_o=<?php echo $id_o?>&usr=<?php echo $usr?>" class="btn btn-danger col-12 fs-5 mb-2">Imprimir Ticket del pedido</a>
